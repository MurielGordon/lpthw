...
class CentralCorridor(Scene):
    def enter(self):
        ...
        if "shoot" in action:
            ...
            costume is flowing and moving around his body, which throws off your aim. Your 
            laser hits his costume but misses him entirely. This completley ruins his brand new 
            costume his mother bought him, which makes him fly into an insane rage and blase you 
...
class LaserWeaponArmory(Scene):
    def enter(self):
        ...
        if guess != code and guesses < 9:
            ...
            import time
            time.sleep(1.8)
            print("Think of a diabolically hot number.")
            guesses += 1
            guess = input("[keypad]> ")
...
class EscapePod(Scene):
    def enter(self):
        ...
        if int(guess) != good_pod:
            print(dedent(f"""
            You jump into pod {guess} and hit the eject button. The pod escaped out into the void of space, then
            implodes as the hull ruptures, crushing your body into jam jelly.
            """))
            return 'death'
        else:
            print(dedent(f"""
            You jump into pod {guess} and hit the eject button. The pod easilt slides out into space heading to
